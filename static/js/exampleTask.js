...
document.addEventListener('DOMContentLoaded', function () {
    ...
    async function loadTasks() {
        try {
            const response = await fetch('show_tasks.py');
            ...
            tasks = data.map(task => ({
                ...
                completed_dates: task.completed_dates || ''  // ✅ date-wise completion
            }));
            renderTasks();
            updateStats();
    ...
    if (saveTaskBtn) {
        saveTaskBtn.addEventListener('click', async (e) => {
            ...
            try {
                const url = editingTaskId ? 'update_tasks.py' : 'save_task.py';
    ...
    window._toggleTask = function(taskId, viewDateStr) {
        const task = tasks.find(t => t.task_id === taskId);
        if (!task) return;

        // ✅ Recurring: date-specific completion check karo
        const alreadyDone = isTaskCompletedOnDate(task, viewDateStr);
        const newStatus   = alreadyDone ? 'pending' : 'completed';

        fetch('update_task_status.py', {
            method: 'POST',
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
            body: `taskId=${taskId}&status=${newStatus}&viewDate=${viewDateStr}&userId=1`
        })
        .then(r => r.json())
        .then(result => {
            if (result.success) {
    ...
    function renderDailyView() {
        ...
        // ✅ Recurring: sirf us date ki completion check karo, global status nahi
        const dayTasks = getTasksForDate(viewDateStr).filter(t => !isTaskCompletedOnDate(t, viewDateStr));
        ...
        timeline.innerHTML = dayTasks.map(task => `
            <div class="timeline-slot" data-id="${task.task_id}" style="display:flex;align-items:flex-start;gap:12px;padding:14px 16px;background:var(--card-bg,#1e2235);border-radius:12px;margin-bottom:10px;border:1px solid var(--border-color,rgba(255,255,255,0.08))">
                ...
                <!-- Checkbox -->
                <div onclick="window._toggleTask(${task.task_id}, '${viewDateStr}')"
    ...
    // ✅ Recurring tasks ke liye date check
    function getTasksForDate(dateStr) {
        return tasks.filter(task => {
            ...
            if (checkDate < taskDate) return false;  // task start hone se pehle mat dikhao
            ...
            if (task.recurring === 'yearly')  return taskDate.getDate()  === checkDate.getDate() &&
                                                     taskDate.getMonth() === checkDate.getMonth();
            return false;
        });
    }

    // ✅ Kisi task ka kisi specific date pe completion status check karo
    function isTaskCompletedOnDate(task, dateStr) {
        if (!task.recurring || task.recurring === 'once') {
            // Non-recurring: DB status use karo
            return task.status === 'completed';
        }
        // Recurring: completed_dates me check karo
        if (!task.completed_dates) return false;
        const doneList = task.completed_dates.split(',').map(d => d.trim());
        return doneList.includes(dateStr);
    }
